...
import {
  ...
  Edit2, Trash2, Guitar, BookOpen, User, X, ChevronDown
} from 'lucide-react';
...
const WorshipPage = () => {
  ...
  useEffect(() => { fetchSongs(); }, [categoryFilter]);

  const handleSearch = () => fetchSongs();

  const openNew = () => {
    setEditSong({ title: '', artist: '', category: 'Adoração', youtube_url: '', lyrics: '', chords: '', composer: '', tone: '' });
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editSong.id) {
        await api.put(`/api/church/worship/${editSong.id}`, editSong);
        toast.success('Louvor atualizado!');
      } else {
        await api.post('/api/church/worship', editSong);
              ...
              {detailSong.youtube_url && (
                <div className="aspect-video rounded-xl overflow-hidden bg-muted">
                  {getYoutubeId(detailSong.youtube_url) ? (
                    <iframe
                      src={`https://www.youtube.com/embed/${getYoutubeId(detailSong.youtube_url)}`}
